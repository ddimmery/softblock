---
title: SoftBlock Demo (Precinct Randomization)
...
In this demo, I'm going to walk through an example experimental design using methods from our [recent paper on Balanced Design](https://arxiv.org/abs/2010.11332).

In particular, I'm going to imagine that I'm designing a test in which I assign different treatments to particular precincts in North Carolina. In order to optimize power, of course, we want to make sure that our two test groups look as similar as possible in terms of prior voting patterns.
...
5. Fake some data and analyze it for average and heterogeneous treatment effects.

# Implementation of methods {.tabset}

## Description

The relevant API is a function with `tidyverse` semantics called `assign_softblock` (or `assign_greedy_neighbors`). These functions accept a vector of columns to be used in the design. The SoftBlock version additionally accepts two arguments, `.s2` for the bandwidth of the RBF kernel to use in the construction of a similarity matrix as well as `.neighbors` which indicates the number of nearest neighbors to include in the graph on which to construct the spanning tree. These parameters do not likely need to be modified.

## Source Code

```{r print_source}
writeLines(readLines("r_implementation.R"))
```
...
```
# Assign treatment four ways {.tabset}
...
message(df_joined$treatment_nn[1])
message(mean(df_joined$treatment_nn))
...
```
## Simulate each design {.tabset}
### Softblock
```{r power_sb}
simulate_power = create_power_simulator(softblock_weights, df_joined$treatment_sb, df_joined$dem_share_pres)
estimate_power_for_effect = function(effect) mean(replicate(2500, simulate_power(effect=effect)))
...
```{r make_outcome}
df_joined$outcome = 0 * rnorm(nrow(df_joined)) + df_joined$dem_share_pres
df_joined$ite = with(df_joined, 0.005 + 0.005 * (plogis((dem_share_pres - median(dem_share_pres)) / sd(dem_share_pres))) - 0.5)
...
geom_histogram(aes(x=hte_sb, fill='SoftBlock'), bins=10, alpha=0.4) +
geom_histogram(aes(x=hte_nn, fill='Greedy Neighbors'), bins=10, alpha=0.4) +
geom_histogram(aes(x=hte_qb, fill='QuickBlock'), bins=10, alpha=0.4) +
scale_x_continuous("Effect (pp)", labels=scales::percent) +
theme_minimal()
```
